...
namespace Eyue.Business.WxPay
{
    public  class WxPayApi
    {
       ...
       */
        public  WxPayData Micropay(WxPayData inputObj, int timeOut = 10)
        {
            ...
            result.FromXml(response, WxPayConfig.KEY);
        ...
        */
        public  WxPayData Refund(WxPayData inputObj, int timeOut = 6,string pvPath="")
        {
            ...
            string response = HttpService.Post(xml, url, true, timeOut, pvPath);//调用HTTP通信接口提交数据到API
        ...
        */
        public  WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6)
        {
            ...
            //签名
            inputObj.SetValue("sign", inputObj.MakeSign(WxPayConfig.KEY));
            string xml = inputObj.ToXml();
	    ...
	    */
        private  void ReportCostTime(string interface_url, int timeCost, WxPayData inputObj)
        {
            //如果不需要进行上报
